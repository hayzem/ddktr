...
QTindA = dblarr(16)                         ; initial induced charges due to trapped electrons
QTindC = dblarr(16)
QTindST = dblarr(5)
cnt=0

QA_ind_e  = dblarr(16,1000)        ; Initial induced Charge on the anode site
QC_ind_e  = dblarr(16,1000)       ; Initial induced Charge on the cathode site
QST_ind_e = dblarr(5,1000)         ; Initial induced Charge on the steering electodes
...
IF lastpos eq 0 THEN BEGIN
   WHILE z ne 0 DO BEGIN
      ...
      x=floor(xe_actual[n_elements(xe_actual)-1]/0.005)
      z=z-1
      ze_actual=[ze_actual,z*0.005]
      FOR i=0,15 DO BEGIN
         QA_ind_e[i,cnt+1] = Qr_e*WP_Ano[i,x,z] + QTindA[i]            ; Final induced charge on anode site
